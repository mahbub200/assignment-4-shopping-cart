...
import { addToCart, removeFromCart, clearCart, getCartItems, getTotalAmount, increaseQuantity, decreaseQuantity, applyDiscount, removeDiscount } from "./cart.js";
...
function displayProductList() {
  ...
  products.forEach((product) => {
    ...
    productCard.style.backgroundColor = "#A08D89";
...
function handleApplyDiscount(productId) {
  ...
  if (discountPercentage > 20) {
    alert("Maximum discount percentage allowed is 20%.");
    return;
  }
...
function displayCartItems() {
  ...
  if (cartItems.length === 0) {
    ...
    cartItems.forEach((item) => {
      ...
      cartItemDiv.style.backgroundColor = "#A08D89";
      ...
      removeButton.addEventListener("click", () => handleRemoveFromCart(item.product.id));
      ...
      decreaseButton.addEventListener("click", () => handleDecreaseQuantity(item.product.id));
      ...
      increaseButton.addEventListener("click", () => handleIncreaseQuantity(item.product.id));
      ...
      discountInput.id = `discount-input-${item.product.id}`;
      ...
      removeDiscountButton.addEventListener("click", () => handleRemoveDiscount(item.product.id));

      const applyDiscountButton = document.createElement("button");
      applyDiscountButton.className = "btn btn-primary";
      applyDiscountButton.textContent = "Apply Discount";
      applyDiscountButton.addEventListener("click", () => handleApplyDiscount(item.product.id));

      discountForm.appendChild(discountInput);
      discountForm.appendChild(applyDiscountButton);
